...
TEST(Unicode, UCS2UTF8Test)
{
   ...
   u8ch[2] = 0;
   EXPECT_STREQ(reinterpret_cast<const char*>(u8"\u00A3"), u8ch) << "Wide pound sign character should convert into a two-byte UTF-8 character";

   // zzzzyyyy yyxxxxxx             1110zzzz    10yyyyyy    10xxxxxx
   ASSERT_EQ(3, ucs2utf8(L'\uF922', u8ch)) << "A Kanji character should convert into a three-byte UTF-8 character";
   u8ch[3] = 0;
   EXPECT_STREQ(reinterpret_cast<const char*>(u8"\uF922"), u8ch) << "A wide Kanji character should convert into a three-byte UTF-8 character";
   ...
   u8ch[4] = 0;
   EXPECT_STREQ(reinterpret_cast<const char*>(u8"\U00010334"), u8ch) << "Wide Gothic character should convert into a fout-byte UTF-8 character";
#endif
...
TEST(Unicode, UTF8CharSizeTest)
{
   ...
   EXPECT_EQ(2, utf8size(reinterpret_cast<const char*>(u8"\u00A3"))) << "A pound sign character is a two-byte UTF-8 character";
   EXPECT_EQ(2, utf8size(reinterpret_cast<const char*>(u8"\u0713"))) << "A Syriac character is a two-byte UTF-8 character";

   // U+0800   .. U+0FFF   E0          A0..BF      80..BF
   EXPECT_EQ(3, utf8size(reinterpret_cast<const char*>(u8"\u0985"))) << "A Bengali character is a three-byte UTF-8 character";
   EXPECT_EQ(3, utf8size(reinterpret_cast<const char*>(u8"\u0F13"))) << "A Tibetan character is a three-byte UTF-8 character";

   // U+1000   .. U+CFFF   E1..EC      80..BF      80..BF
   EXPECT_EQ(3, utf8size(reinterpret_cast<const char*>(u8"\u1003"))) << "A Myanmar character is a three-byte UTF-8 character";
   EXPECT_EQ(3, utf8size(reinterpret_cast<const char*>(u8"\uCF03"))) << "A Hangul Syllable character is a three-byte UTF-8 character";

   // U+D000   .. U+D7FF   ED          80..9F      80..BF
   EXPECT_EQ(3, utf8size(reinterpret_cast<const char*>(u8"\uD003"))) << "A Hangul Syllable character is a three-byte UTF-8 character";
   EXPECT_EQ(3, utf8size(reinterpret_cast<const char*>(u8"\uD78F"))) << "A Hangul Syllable character is a three-byte UTF-8 character";

   // U+E000   .. U+FFFF   EE..EF      80..BF      80..BF
   EXPECT_EQ(3, utf8size(reinterpret_cast<const char*>(u8"\uE014"))) << "A Private Use Area character is a three-byte UTF-8 character";
   EXPECT_EQ(3, utf8size(reinterpret_cast<const char*>(u8"\uF8FB"))) << "A Private Use Area character is a three-byte UTF-8 character";

   // U+10000  .. U+3FFFF  F0          90..BF      80..BF      80..BF
   EXPECT_EQ(4, utf8size(reinterpret_cast<const char*>(u8"\U00010104"))) << "An Aegean Numbers character is a four-byte UTF-8 character";
   EXPECT_EQ(4, utf8size(reinterpret_cast<const char*>(u8"\U0002F947"))) << "A CJK Compatibility Ideographs Supplement character is a four-byte UTF-8 character";

   // U+40000  .. U+FFFFF  F1..F3      80..BF      80..BF      80..BF
   EXPECT_EQ(4, utf8size(reinterpret_cast<const char*>(u8"\U00040000"))) << "A character in U+40000..U+FFFFF range is a four-byte UTF-8 character";
   EXPECT_EQ(4, utf8size(reinterpret_cast<const char*>(u8"\U000FFFFF"))) << "A character in U+40000..U+FFFFF range is a four-byte UTF-8 character";

   // U+100000 .. U+10FFFF F4          80..8F      80..BF      80..BF   
   EXPECT_EQ(4, utf8size(reinterpret_cast<const char*>(u8"\U00100000"))) << "A Supplementary Private Use Area-B character is a four-byte UTF-8 character";
   EXPECT_EQ(4, utf8size(reinterpret_cast<const char*>(u8"\U0010FFFD"))) << "A Supplementary Private Use Area-B character is a four-byte UTF-8 character";
}
...
TEST(Unicode, UCS2toUTF8StringLengthTest)
{
   ...
   u8out[3] = 0;
   EXPECT_STREQ(reinterpret_cast<const char*>(u8"ABC"), u8out);
   ...
   u8out[4] = 0;
   EXPECT_STREQ(reinterpret_cast<const char*>(u8"A\u00A3C"), u8out);
   ...
   u8out[5] = 0;
   EXPECT_STREQ(reinterpret_cast<const char*>(u8"A\uF922C"), u8out);
...
TEST(Unicode, UCS2toUTF8StrLenSurPairTest)
{
   ...
   u8out[5] = 0;
   EXPECT_STREQ(reinterpret_cast<const char*>(u8"A\uFFFDC"), u8out) << "Output should contain the replacement character instead the high surrogate pair";
   ...
   ASSERT_EQ('X', u8out[5]) << "The character past allowed input should not change";
   u8out[5] = 0;
   EXPECT_STREQ(reinterpret_cast<const char*>(u8"A\uFFFDC"), u8out) << "Output should contain the replacement character instead the low surrogate pair";
