...
//
// Using a pointer to a formatting function in the format method makes it possible 
// to use different formatting functions with the same formatter and buffer.
//
...
class buffer_formatter_t {
   public:
      //
      // A formatting function takes a buffer object that will hold formatted text after
      // the function returns. The function should return the number of bytes in the new
      // formatted text, including the null character, if there is one. The formatter 
      // does not interpret or modify the formatted string in any way. There is no error 
      // value reserved and the function should throw an exception in case of any error.
      //
      typedef size_t (*format_cb_t)(string_t::char_buffer_t&, format_params_t ...);
   ...
   public:
      ...
      string_t::char_buffer_t format(format_cb_t formatcb, format_params_t ... args);

      const char *operator () (format_cb_t formatcb, format_params_t ... args);
