...
collected from three distinct sources:
  ...
  a) LogFile variables in the default configuration file and its
     includes.
  b) Command line, configuration files specified with the `-c` option
     and its includes.
  c) If the `--pipe-log-names` option was used, log file names are
...
the in their log record time stamp order. This feature is intended for
